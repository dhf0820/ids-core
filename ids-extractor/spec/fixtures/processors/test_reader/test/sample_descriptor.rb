  ...
  class SampleDescriptor < Reader::Descriptor
    ...
    def self.recognize?(document)
			puts "  @@@ Sample Recognize #{document.inspect}"
      status = document.first_identifier?("EMERGENCY DEPARTMENT VISIT", :line => 2) and
      document.other_identifier?("Care Provider", :line => 4)  #really is line 5 but test is to fail
			puts "   @@@ Sample Returned #{status}"
			status
    end
...
#      	extract_text :account_num, :length => 7
      ...
      def extract_cc
      	...
      	r, c = find_prompt "cc:", :row => r + 1 unless(r.nil?)
      end
  end
