class TestEr < Descriptor
    ...
    def self.recognize?(document)
      ...
      document.other_identifier?("Care Provider", :line => 4)
    end
...
#puts "Assigning a new Subclass = #{data_element(:subclass).name}"
    ...
    def add_data_elements
      DataElements[:subclassid] = Reader::DataElement.new('subclassid', 11, :alphanumeric)
      DataElements[:class] = Reader::DataElement.new('class', 11, :alphanumeric)
